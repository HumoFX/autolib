...
class AbstractHuman(models.Model):
    ...
    def get_display_name(self):
        """Return author display full name"""
        if self.first_name:
...
class Book(models.Model):
    udc = models.ForeignKey(Category, on_delete=models.CASCADE, related_name='УДК', null=True,
                            verbose_name="УДК", help_text="Временно", blank=True)
    udc_new = models.ForeignKey(UDC, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="УДК*",
    ...
    img = models.ImageField(upload_to='img/books', verbose_name='Обложка(Фото)', null=True, blank=True)
    ...
    date_get = models.DateField(auto_now_add=False, verbose_name='Дата получения', null=True, blank=True)
        ...
        for i in s:
            if i.__contains__('-'):
                ...
                if start.isnumeric() and end.isnumeric():
                    ...
                    iters.extend(i)
        integer_map = map(int, s)
        inventory_list = list(integer_map)
        inventory_list.extend(iters)
